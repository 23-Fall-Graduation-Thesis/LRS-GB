...
from scheduler.LRS_GB import LRS_GB_Score
...
class LRS_GB_Score_Trainer(TrainerBase):
    ...
    def train_model(self, epochs, init_lr):
        ...
        with open(epoch_log_filename, 'a', newline='') as f:
            wr = csv.writer(f)
            # epoch, trial, target_initweva, tryweva, try_initweva, current_LR
            wr.writerow(['epoch', 'trial', 'target_initweva', 'tryweva', 'try_initweva', 'current_lr'])
            ...
            while Trial_error:
                ...
                if trial > 50:
                    ...
                    with open(f"./results/trial.csv", 'a', newline='') as f:
                        wr = csv.writer(f)
                        if self.increase_bound:
                            wr.writerow(['autoGB', model_name, dataset, epoch, '-', '-', '-', self.thr_init_score, self.K, self.scale_factor, self.inc_type, self.log_time])
                        else:
                            wr.writerow(['autoGB', model_name, dataset, epoch, '-', '-', '-', self.thr_init_score, self.K, self.scale_factor, '-', self.log_time])
                    exit()
                ...
                # weight variation between previous epoch model
                weva_try = self.get_weva(self.model, model_try, lr_scheduler.layer_name_dict)
                ...
                if math.isnan(train_loss):
                    ...
                    with open(f"./results/nan.csv", 'a', newline='') as f:
                        ...
                        if self.increase_bound:
                            wr.writerow(['GB', model_name, dataset, epoch, '-', '-', '-', self.thr_init_score, self.K, self.scale_factor, self.inc_type, init_weva_try, weva_try[:-1], optimizer_try_lrs[:-1], self.log_time])
                        else:
                            wr.writerow(['GB', model_name, dataset, epoch, '-', '-', '-', self.thr_init_score, self.K, self.scale_factor, '-', init_weva_try, weva_try[:-1], optimizer_try_lrs[:-1], self.log_time])
                    exit()
                ...
                if Trial_error == False:
                    ...
                    manager.record(epoch, now_lr[:-1], init_weva_try[:-1])
                else:
                    weva_table.append(weva_try)
                    init_weva_table.append(init_weva_try)
                    lr_table.append(optimizer_try_lrs)
                    now_lr = lr_scheduler.adjustLR(weva_table, init_weva_try, init_diff_table, lr_table, epoch, param_num_list)
                ...
                if Trial_error == False:
                    with open(epoch_log_filename, 'a', newline='') as f:
                        wr = csv.writer(f)
                        # epoch, trial, target_initweva, tryweva, try_initweva, current_learning rate
